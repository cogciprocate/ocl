...
fn main() {
	if cfg!(windows) {
		...
		if let Ok(sdk) = std::env::var("INTELOCLSDKROOT") {
		...
		if let Ok(sdk) = std::env::var("CUDA_PATH") {
			let mut path = std::path::PathBuf::from(sdk);
			path.push("lib");
		...
		if let Ok(sdk) = std::env::var("AMDAPPSDKROOT") {
			let mut path = std::path::PathBuf::from(sdk);
			path.push("lib");
			path.push(if cfg!(target_arch="x86_64") { "x64_64" } else { "x86"});
			println!("cargo:rustc-link-search=native={}", path.display());
		}
	}
}
