...
fn buffer_ops_rect() {
    ...
    let buf = Buffer::new(proque.queue(),
    ...
    for idx in 0..proque.dims().to_len() {
        ...
        // print!("[{:02}]", vec[i]);
        // if i % 20 == 19 { print!("\n"); }
        assert!(vec[idx] == ADDEND * ttl_runs as f32, "vec[{}]: {}", idx, vec[idx]);
    }
    ...
    tests::verify_vec_rect([0, 0, 0],
                           dims,
                           ADDEND * ttl_runs as f32,
                           ADDEND * (ttl_runs - 1) as f32,
                           dims,
                           1,
                           &vec,
                           ttl_runs,
    ...
    for _ in 0..TEST_ITERS {
        ...
        // Reset vec:
        unsafe {
            core::enqueue_read_buffer_rect(proque.queue(),
                                           &buf,
                                           true,
                                           [0, 0, 0],
                                           [0, 0, 0],
        ...
        // Verify:
        tests::verify_vec_rect(vec_origin, read_region, cur_val, old_val, dims, 1, &vec, ttl_runs, false).unwrap();
        ...
        buf.cmd()
            .read(&mut vec)
            .rect([0, 0, 0], [0, 0, 0], dims, row_pitch, slc_pitch, row_pitch, slc_pitch)
            .queue(proque.queue())
            .block(true)
            .enq()
            .unwrap();
    ...
    for _ in 0..TEST_ITERS {
        ...
        core::enqueue_write_buffer_rect(proque.queue(),
                                        &buf,
                                        false,
                                        buf_origin,
                                        vec_origin,
        ...
        tests::verify_vec_rect(buf_origin, read_region, nxt_val, cur_val, dims, 1, &vec, ttl_runs, true).unwrap();
    ...
    let buf_src = Buffer::new(proque.queue(),
                              Some(core::MEM_READ_WRITE | core::MEM_COPY_HOST_PTR),
                              proque.dims().clone(),
                              Some(&vec_src))
        .unwrap();
    ...
    let buf_dst = Buffer::new(proque.queue(),
                              Some(core::MEM_READ_WRITE | core::MEM_COPY_HOST_PTR),
                              proque.dims().clone(),
                              Some(&vec_dst))
        .unwrap();
    ...
    for _ in 0..TEST_ITERS {
        ...
        core::enqueue_copy_buffer_rect::<f32>(proque.queue(),
                                              &buf_src,
                                              &buf_dst,
                                              src_origin,
                                              dst_origin,
        ...
        // Verify that our random region was in fact written correctly:
        tests::verify_vec_rect(dst_origin, read_region, nxt_val, cur_val, dims, 1, &vec_dst, ttl_runs, true).unwrap();
