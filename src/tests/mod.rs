...
fn gen_region_origin(dims: &[usize; 3]) -> ([usize; 3], [usize; 3]) {
    ...
    let region = [rng.gen_range(1, dims[0] + 1), rng.gen_range(1, dims[1] + 1), rng.gen_range(1, dims[2] + 1)];

    let origin = [rng.gen_range(0, (dims[0] - region[0]) + 1),
...
fn verify_vec_rect<T: OclScl>(origin: [usize; 3],
                              region: [usize; 3],
                              in_region_val: T,
                              out_region_val: T,
                              vec_dims: [usize; 3],
    ...
    if print {
        println!("Verifying run: '{}', origin: {:?}, region: {:?}, vec_dims: {:?}",
                 ttl_runs,
                 origin,
                 region,
                 vec_dims);
