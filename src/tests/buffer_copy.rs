...
fn buffer_copy_core() {
    ...
    core::build_program(&program, &[device_id], &CString::new("").unwrap(), None, None).unwrap();
    ...
    // Run the kernel:
    core::enqueue_kernel(&queue, &kernel, 1, None, &dims, None, None, None).unwrap();

    // Copy src_buffer to dst_buffer:
    let copy_range = (153, 150000);
    core::enqueue_copy_buffer::<f32>(&queue,
                                     &src_buffer,
                                     &dst_buffer,
                                     copy_range.0,
                                     copy_range.0,
    ...
    unsafe { core::enqueue_read_buffer(&queue, &src_buffer, true, 0, &mut src_buffer_vec, None, None).unwrap() };
    // Read results from dst_buffer:
    unsafe { core::enqueue_read_buffer(&queue, &dst_buffer, true, 0, &mut dst_buffer_vec, None, None).unwrap() };
