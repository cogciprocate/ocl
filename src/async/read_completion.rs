...
use async::{Error as AsyncError, Result as AsyncResult, RwGuard, PendingRwGuard};
...
impl<T> ReadCompletion<T> where T: OclPrm {
    ...
    fn new(data: RwGuard<T>, triggerable_event: Option<Event>)
            -> ReadCompletion<T>
    {
        ReadCompletion {
            data: data,
...
impl<T> Deref for ReadCompletion<T> where T: OclPrm {
    type Target = RwGuard<T>;

    fn deref(&self) -> &RwGuard<T> {
        &self.data
    }
}

impl<T> DerefMut for ReadCompletion<T> where T: OclPrm {
    fn deref_mut(&mut self) -> &mut RwGuard<T> {
        &mut self.data
    }
}
...
pub struct FutureReadCompletion<T> {
    read_event: Event,
    triggerable_event: Option<Event>,
    data: Option<PendingRwGuard<T>>
}

#[allow(dead_code)]
impl<T> FutureReadCompletion<T> where T: OclPrm {
    pub fn new(data: PendingRwGuard<T>, read_event: Event) -> FutureReadCompletion<T> {
...
impl<T> Future for FutureReadCompletion<T> where T: OclPrm {
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        ...
        // self.data.poll().map(|rslt| rslt.map(|rw_guard| ))
    }
}
