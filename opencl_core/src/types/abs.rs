...
use libc;
use ffi::cl_h::{cl_platform_id, cl_device_id,  cl_context, cl_command_queue, cl_mem, cl_program,
...
impl Clone for Context {
    fn clone(&self) -> Context {
        unsafe { ::retain_context(self).unwrap(); }
        Context(self.0)
    }
}
...
impl Drop for Context {
    fn drop(&mut self) {
        unsafe { ::release_context(self).ok(); }
    }
}
...
impl CommandQueue {
    ...
    pub unsafe fn from_copied_ptr(ptr: cl_command_queue) -> CommandQueue {
        ...
        ::retain_command_queue(&copy).unwrap();
        copy
    }
...
impl Clone for CommandQueue {
    fn clone(&self) -> CommandQueue {
        unsafe { ::retain_command_queue(self).unwrap(); }
        CommandQueue(self.0)
    }
}
...
impl Drop for CommandQueue {
    fn drop(&mut self) {
        unsafe { ::release_command_queue(self).ok(); }
    }
}
...
impl Clone for Mem {
    fn clone(&self) -> Mem {
        unsafe { ::retain_mem_object(self).unwrap(); }
        Mem(self.0)
    }
}
...
impl Drop for Mem {
    fn drop(&mut self) {
        unsafe { ::release_mem_object(self).ok(); }
    }
}
...
impl Clone for Program {
    fn clone(&self) -> Program {
        unsafe { ::retain_program(self).unwrap(); }
        Program(self.0)
    }
}
...
impl Drop for Program {
    fn drop(&mut self) {
        unsafe { ::release_program(self).ok(); }
    }
}
...
//         unsafe { ::release_program(self.obj_core).unwrap(); }
//     }
// }
...
impl Clone for Kernel {
    fn clone(&self) -> Kernel {
        unsafe { ::retain_kernel(self).unwrap(); }
        Kernel(self.0)
    }
}
...
impl Event {
    ...
    pub unsafe fn from_cloned_ptr(ptr: cl_event) -> OclResult<Event> {
        ...
        if new_core.is_valid() {
            try!(::retain_event(&new_core));
            Ok(new_core)
        } else {
            OclError::err("::Event::from_cloned_ptr: Invalid pointer `ptr`.")
        }
    }
...
impl Clone for Event {
    fn clone(&self) -> Event {
        unsafe { ::retain_event(self).expect("::Event::clone"); }
        Event(self.0)
    }
}
...
impl EventList {
    ...
    /// count (with `::retain_event`) then letting `event` drop which just decrements it right back.
    ...
    /// you will manage its associated reference count carefully.
    pub unsafe fn as_ptr_ref(&self) -> &cl_event {
        self.event_ptrs.first().expect("ocl::::EventList::as_ptr_ref(): \
    ...
    pub fn clear_completed(&mut self) -> OclResult<()> {
        ...
        for (idx, event_ptr) in self.event_ptrs.iter().enumerate() {
            let status = try!(::get_event_status(&EventRefWrapper(event_ptr, 1)));
        ...
        for &idx in &cmpltd_events {
            unsafe {
                try!(::release_event(&EventRefWrapper(&self.event_ptrs[idx], 1)));
            }
        }
    ...
    fn clone(&self) -> EventList {
        for event_ptr in &self.event_ptrs {
            if !(*event_ptr).is_null() {
                unsafe { ::retain_event(&EventRefWrapper(event_ptr, 1))
                    .expect("::EventList::clone") }
            }
        }
...
impl Drop for EventList {
    fn drop(&mut self) {
        ...
        for event_ptr in &self.event_ptrs {
            unsafe { ::release_event(&EventRefWrapper(event_ptr, 1)).ok(); }
...
impl Clone for Sampler {
    fn clone(&self) -> Sampler {
        unsafe { ::retain_sampler(self).unwrap(); }
        Sampler(self.0)
    }
}
...
impl Drop for Sampler {
    fn drop(&mut self) {
        unsafe { ::release_sampler(self).ok(); }
    }
}
