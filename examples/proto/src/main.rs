...
const WORK_SIZE: usize = 1 << 23;

// Number of times to run the loop:
const TASK_ITERS: usize = 20;
...
pub fn main() {
    ...
    for task_iter in 0..TASK_ITERS {
        ...
        let write = future_write_data.and_then(move |mut data| {
            ...
            printlnc!(green_bold: "Mapped write complete (iter: {}). ", task_iter);

            // When `data` (a `MemMap`) goes out of scope here it
            // automatically attempts to unmap itself unless it has already
            // been unmapped. We can also call it manually if we needed to
            // pass it a new queue or events (demonstrated below).
        ...
        printlnc!(green: "Mapped write enqueued (iter: {}).", task_iter);
        ...
        printlnc!(green: "Kernel enqueued (iter: {}).", task_iter);
        ...
        let read = future_read_data.and_then(move |mut data| {
                ...
                printlnc!(green_bold: "Mapped read and verify complete (iter: {}). ", task_iter);
        ...
        printlnc!(green: "Mapped read enqueued (iter: {}).", task_iter);
